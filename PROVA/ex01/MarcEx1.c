...
int insere_ArvAVL(ArvAVL *raiz, int valor, int s, int t, int c){
    int res;
    ...
    struct NO *atual = *raiz;
    if(valor < atual->info){
        if((res = insere_ArvAVL(&(atual->esq), valor, s, t, c)) == 1){
        ...
        if(valor > atual->info){
            if((res = insere_ArvAVL(&(atual->dir), valor, s, t, c)) == 1){
                if(fatorBalanceamento_NO(atual) >= 2){
...
int main (){
    ...
    int v, s, t, c;
    while (scanf("%d %d %d %d", &v, &s, &t, &c) != EOF){

        insere_ArvAVL(avl, v, s, t, c);
    }

    emOrdem_ArvAVL(avl);
    printf("%d", altura_ArvAVL(avl));
